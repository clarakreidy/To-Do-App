...
class CalendarViewController: UIViewController, UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell.Status.isChecked = rows[indexPath.row].Checked
        return cell
    }
    ...
    func loadTasks(section: String) {
        rows = []
        if (section == "Today") {
            let today = TASKS.filter { (task: Todo) -> Bool in
        ...
        if (section == "Tomorrow") {
            let tomorrow = TASKS.filter { (task: Todo) -> Bool in
        ...
        if (section == "This Week") {
            ...
            let thisWeek = TASKS.filter { (task: Todo) -> Bool in
        ...
        if (section == "Later") {
            ...
            let later = TASKS.filter { (task: Todo) -> Bool in
